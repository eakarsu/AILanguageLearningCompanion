// Auto-generated feature route: voice-tutor
// Domain: speaking coach + pronunciation evaluator — Capability: scoring pronunciation and giving feedback
// Mounted under: /api/voice-tutor
...
// POST /api/voice-tutor/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a speaking coach + pronunciation evaluator. Your task is scoring pronunciation and giving feedback. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/voice-tutor/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/voice-tutor/:id — fetch one
...
// DELETE /api/voice-tutor/:id — discard a run
