// Auto-generated feature route: vertical-vocab-pack
// Domain: domain vocabulary curator — Capability: generating medical/legal/business vocabulary packs
// Mounted under: /api/vertical-vocab-pack
  ...
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-Title': 'Language Learning Companion'
      }
    };
...
// POST /api/vertical-vocab-pack/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a domain vocabulary curator. Your task is generating medical/legal/business vocabulary packs. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vertical-vocab-pack/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vertical-vocab-pack/:id — fetch one
...
// DELETE /api/vertical-vocab-pack/:id — discard a run
