...
// Route: /api/gap-community
// Feature: Community
// Description: Community forum / peer practice rooms
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Community. Community forum / peer practice rooms' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'community', title: 'Community', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'community', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['community']);
      return res.json({ slug: 'community', items: r.rows || [] });
    }
    return res.json({ slug: 'community', items: [] });
  } catch (e) {
    return res.json({ slug: 'community', items: [], error: (e && e.message) || 'Server error' });
  }
});
