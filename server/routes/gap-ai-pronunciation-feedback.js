...
// Route: /api/gap-ai-pronunciation-feedback
// Feature: Ai Pronunciation Feedback
// Description: (evaluate spoken input via ASR)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Pronunciation Feedback. (evaluate spoken input via ASR)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Pronunciation Feedback\nDescription: (evaluate spoken input via ASR)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-pronunciation-feedback', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-pronunciation-feedback', title: 'Ai Pronunciation Feedback', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-pronunciation-feedback', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-pronunciation-feedback']);
      return res.json({ slug: 'ai-pronunciation-feedback', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-pronunciation-feedback', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-pronunciation-feedback', items: [], error: (e && e.message) || 'Server error' });
  }
});
