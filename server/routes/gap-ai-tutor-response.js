...
// Route: /api/gap-ai-tutor-response
// Feature: Ai Tutor Response
// Description: (interactive 1-on-1 conversation)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Tutor Response. (interactive 1-on-1 conversation)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Tutor Response\nDescription: (interactive 1-on-1 conversation)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-tutor-response', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-tutor-response', title: 'Ai Tutor Response', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-tutor-response', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-tutor-response']);
      return res.json({ slug: 'ai-tutor-response', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-tutor-response', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-tutor-response', items: [], error: (e && e.message) || 'Server error' });
  }
});
