...
// Route: /api/gap-speech-to-text
// Feature: Speech To Text
// Description: Speech-to-text capture in the client
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Speech To Text. Speech-to-text capture in the client' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'speech-to-text', title: 'Speech To Text', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'speech-to-text', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['speech-to-text']);
      return res.json({ slug: 'speech-to-text', items: r.rows || [] });
    }
    return res.json({ slug: 'speech-to-text', items: [] });
  } catch (e) {
    return res.json({ slug: 'speech-to-text', items: [], error: (e && e.message) || 'Server error' });
  }
});
