...
// Route: /api/gap-ai-personalized-lesson-plan
// Feature: Ai Personalized Lesson Plan
// Description: (adaptive difficulty by progress)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Personalized Lesson Plan. (adaptive difficulty by progress)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Personalized Lesson Plan\nDescription: (adaptive difficulty by progress)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-personalized-lesson-plan', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-personalized-lesson-plan', title: 'Ai Personalized Lesson Plan', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-personalized-lesson-plan', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-personalized-lesson-plan']);
      return res.json({ slug: 'ai-personalized-lesson-plan', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-personalized-lesson-plan', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-personalized-lesson-plan', items: [], error: (e && e.message) || 'Server error' });
  }
});
