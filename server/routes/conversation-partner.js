// Auto-generated feature route: conversation-partner
// Domain: agentic language conversation partner — Capability: steering topics and escalating difficulty
// Mounted under: /api/conversation-partner
...
// POST /api/conversation-partner/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic language conversation partner. Your task is steering topics and escalating difficulty. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/conversation-partner/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/conversation-partner/:id — fetch one
...
// DELETE /api/conversation-partner/:id — discard a run
