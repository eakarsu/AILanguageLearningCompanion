// Auto-generated feature route: cultural-storytelling
// Domain: cultural narrative tutor — Capability: producing immersive stories with mid-narrative quizzes
// Mounted under: /api/cultural-storytelling
...
// POST /api/cultural-storytelling/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a cultural narrative tutor. Your task is producing immersive stories with mid-narrative quizzes. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/cultural-storytelling/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/cultural-storytelling/:id — fetch one
...
// DELETE /api/cultural-storytelling/:id — discard a run
