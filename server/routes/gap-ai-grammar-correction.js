...
// Route: /api/gap-ai-grammar-correction
// Feature: Ai Grammar Correction
// Description: (auto-correct writing exercises)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Grammar Correction. (auto-correct writing exercises)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Grammar Correction\nDescription: (auto-correct writing exercises)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-grammar-correction', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-grammar-correction', title: 'Ai Grammar Correction', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-grammar-correction', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-grammar-correction']);
      return res.json({ slug: 'ai-grammar-correction', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-grammar-correction', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-grammar-correction', items: [], error: (e && e.message) || 'Server error' });
  }
});
