...
// Route: /api/gap-certification
// Feature: Certification
// Description: Certification exam prep (TOEFL, IELTS)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Certification. Certification exam prep (TOEFL, IELTS)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'certification', title: 'Certification', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'certification', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['certification']);
      return res.json({ slug: 'certification', items: r.rows || [] });
    }
    return res.json({ slug: 'certification', items: [] });
  } catch (e) {
    return res.json({ slug: 'certification', items: [], error: (e && e.message) || 'Server error' });
  }
});
